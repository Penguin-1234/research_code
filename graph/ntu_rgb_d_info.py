...
class Graph:
    def __init__(self, labeling_mode='spatial', scale=1):
        ...
        self.A_outward_binary = tools_info.get_adjacency_matrix(self.outward, self.num_node)
        self.A = self.get_adjacency_matrix(labeling_mode)
        self.A1 = tools_info.get_spatial_graph(num_node_1, self_link_1, inward_1, outward_1)
        self.A2 = tools_info.get_spatial_graph(num_node_2, self_link_2, inward_2, outward_2)
        self.A_binary = tools_info.edge2mat(neighbor, num_node)
        self.A_norm = tools_info.normalize_adjacency_matrix(self.A_binary + 2*np.eye(num_node))
        self.A_binary_K = tools_info.get_k_scale_graph(scale, self.A_binary)

        self.A_A1 = ((self.A_binary + np.eye(num_node)) / np.sum(self.A_binary + np.eye(self.A_binary.shape[0]), axis=1, keepdims=True))[indices_1]
        self.A1_A2 = tools_info.edge2mat(neighbor_1, num_node_1) + np.eye(num_node_1)
    ...
    def get_adjacency_matrix(self, labeling_mode=None):
        ...
        if labeling_mode == 'spatial':
            A = tools_info.get_spatial_graph(num_node, self_link, inward, outward)
        else:
            raise ValueError()
        return A
